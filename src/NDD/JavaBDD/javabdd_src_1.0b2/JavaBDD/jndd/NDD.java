...
import jdd.bdd.BDD;
import jdd.util.Dot;
import net.sf.javabdd.NDDFactory.bdd;
...
public class NDD {
    ...
    private static NDD ExistRec(NDD a, int field) { // not updated with lazyGC
        ...
        int hash = 0;
        if (table.lazyGC) {
            if (table.existCache.lookup(a))
                return table.existCache.answer;
            hash = table.existCache.hashValue;
        }
        ...
        if (a.field == field) {
            ...
            if (table.lazyGC) {
                toProtect.add(ret);
                table.existCache.insert(hash, a, ret);
            }
            return ret;
        }
    }
    ...
    public NDD satOne(NDD a) {
        return satOne_rec(a);
    }

    private NDD satOne_rec(NDD a) {
        if (a.is_True() || a.is_False())
            return a;
        Map<NDD, Integer> edges = a.edges;
        NDD child = null;
        int edge = 0;
        ...
        NDD result = satOne_rec(child);
        HashMap<NDD, Integer> newEdge = new HashMap<>();
        newEdge.put(result, jddSat);
        return newbdd = new NDD(a.field, newEdge);
    }
